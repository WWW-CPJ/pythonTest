...
class Chapters(models.Model):
    ...
    class Meta:
        managed = False  #该属性为False时，Django不会为该模型创建数据库表
        db_table = 'chapters'
...
class SearchAppSearchitem(models.Model):
    chapter = models.CharField(max_length=200)
    link = models.CharField(max_length=500)

    class Meta:
        managed = False
        db_table = 'search_app_searchitem'

# 使用 ：python manage.py inspectdb > search_app/models.py 进行数据迁移生成模型
# 在 search_project 根目录下执行命令，生成模型文件
